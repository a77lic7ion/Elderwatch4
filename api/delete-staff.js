  ...
  try {
    ...
    try {
      ...
      if (signInData.idToken) {
        // Delete the user by disabling + deleting via admin API
        await fetch(
          `https://identitytoolkit.googleapis.com/v1/projects/${SERVICE_ACCOUNT.project_id}/accounts/${staffId}?key=${API_KEY}`,
          {
            method: 'DELETE',
    ...
    // 2. Delete Firestore staff document
    await fetch(
    ...
    return res.status(500).json({ error: error.message || 'Failed to delete staff' });
  }
}
