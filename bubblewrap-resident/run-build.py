...
TARGET_DIR = "/home/shaun/Hermes Projects/Elderwatch4/bubblewrap-project"
os.chdir(TARGET_DIR)
        ...
        while time.time() - start < timeout:
            ...
            if r:
                try:
                    ...
                    if not sent_y and ("Y/n" in text or "y/n" in text.lower()) and "?" in text:
                        time.sleep(0.3)
                        os.write(master_fd, b"Y\n")
                        sent_y = True
                        print("\n[PTY] Sent: Y (auto-accept)", file=sys.stderr, flush=True)
                    
                    if sent_y and send_jdk and not sent_jdk and "Path to your existing JDK" in text:
                        time.sleep(0.3)
                        os.write(master_fd, f"{JDK_PATH}\n".encode())
                        sent_jdk = True
                        print(f"[PTY] Sent: {JDK_PATH}", file=sys.stderr, flush=True)
                        
                    # Handle password prompts for keystore
                    if "Password" in text and ("Key Store" in text or "key store" in text.lower()):
                        time.sleep(0.3)
                        ...
                        time.sleep(0.3)
                        os.write(master_fd, b"B33tl3sL1lly@123\n")
                        print("[PTY] Sent: key password", file=sys.stderr, flush=True)
